...
extern "C" void ethernetif_input2(struct netif *netif);
...
static bool ensure_safe_temperature(const bool wait=true, const PauseMode mode=PAUSE_MODE_SAME) {
  ...
  //ui.pause_show_message(PAUSE_MESSAGE_HEATING, mode); UNUSED(mode); //Elsan dis.
...
bool load_filament(const_float_t slow_load_length/*=0*/, const_float_t fast_load_length/*=0*/, const_float_t purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (!ensure_safe_temperature(false, mode)) {
    //if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_STATUS, mode); //Elsan dis.
    return false;
  }

  if (pause_for_user) {
    //if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_INSERT, mode); //Elsan dis.
  ...
  //if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_LOAD, mode); //Elsan dis.
  ...
  if (fast_load_length) {
    ...
    do {
      if (purge_length > 0) {
        ...
        //if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_PURGE); //Elsan dis.
...
bool unload_filament(const_float_t unload_length, const bool show_lcd/*=false*/,
  ...
  if (!ensure_safe_temperature(false, mode)) {
    //if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_STATUS); //Elsan dis.
    return false;
  }

  //if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_UNLOAD, mode); //Elsan dis.
  ...
  disable_active_extruder(); //Elsan dis for test.
...
void show_continue_prompt(const bool is_reload) {
  ...
  //ui.pause_show_message(is_reload ? PAUSE_MESSAGE_INSERT : PAUSE_MESSAGE_WAITING); //Elsan dis.
  SERIAL_ECHO_START();
...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  #if ENABLED(PAUSE_PARK_NOZZLE_TIMEOUT)   //Elsan from Xlite
  ...
  KEEPALIVE_STATE(PAUSED_FOR_USER);
  TERN_(HOST_PROMPT_SUPPORT, hostui.prompt_do(PROMPT_USER_CONTINUE, GET_TEXT_F(MSG_NOZZLE_PARKED), FPSTR(CONTINUE_STR))); //Elsan: can dis, HOST_PROMPT_SUPPORT not used.
  //TERN_(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_NOZZLE_PARKED))); //Elsan dis
  wait_for_user = true;    // LCD click or M108 will clear this
  while (wait_for_user) {
    ethernetif_input2(&gnetif); //Elsan
    if(websocket_task_createdN[waiting_task]) websocket_taskN((void *) pcbN[waiting_task], waiting_task); //Elsan
    ...
    if (nozzle_timed_out) {
      //ui.pause_show_message(PAUSE_MESSAGE_HEAT); //Elsan dis.
      ...
      //TERN_(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_HEATER_TIMEOUT))); //Elsan dis
      ethernetif_input2(&gnetif); //Elsan
      if(websocket_task_createdN[waiting_task]) websocket_taskN((void *) pcbN[waiting_task], waiting_task); //Elsan
      ...
      //TERN_(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_REHEATDONE))); //Elsan dis
...
void resume_print(const_float_t slow_load_length/*=0*/, const_float_t fast_load_length/*=0*/, const_float_t purge_length/*=ADVANCED_PAUSE_PURGE_LENGTH*/, const int8_t max_beep_count/*=0*/, const celsius_t targetTemp/*=0*/ DXC_ARGS) {
  ...
  //ui.pause_show_message(PAUSE_MESSAGE_RESUME); //Elsan dis.
  ...
  //ui.pause_show_message(PAUSE_MESSAGE_STATUS); //Elsan dis.
