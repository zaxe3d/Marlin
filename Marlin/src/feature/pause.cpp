...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  # if ENABLED(PAUSE_PARK_NOZZLE_TIMEOUT)
  ...
  #endif
  #if ENABLED(EXTENSIBLE_UI)
    // TODO these onUserConfirmRequired_P breaks dgus screen don't go back to
    // previous screen. Maybe take these to another status text on sd printing page
    // to correctly inform user rather than taking them to info screen. disabling for now
