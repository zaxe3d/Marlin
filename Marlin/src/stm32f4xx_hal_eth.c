...
/* Includes ------------------------------------------------------------------*/
#include "./stm32f4xx_hal_conf.h" //Elsan 8.8.2022
//#include "stm32f4xx_hal_eth.h" //Elsan test

#include "stm32f4xx_hal.h"

//#include "Drivers/CMSIS/Device/ST/STM32F4xx/Include/stm32f407xx.h" //Elsan test
//#include "./stm32f4xx_hal_conf.h" //Elsan 8.8.2022
//#include "stm32f4xx_hal_eth.h" //Elsan test
//#define  HAL_ETH_MODULE_ENABLED //Elsan test
  ...
  if (heth->gState == HAL_ETH_STATE_STARTED)
  {
    ...
    while ((dmatxdesc->DESC0 & ETH_DMATXDESC_OWN) != (uint32_t)RESET)
    {
      if ((heth->Instance->DMASR & ETH_DMASR_FBES) != (uint32_t)RESET)
      {
        ...
        /* Return function status */
        //prnt("HAL_ETH_Transmit: HAL_ERROR", 27);
        return HAL_ERROR;
      }
      ...
      if (Timeout != HAL_MAX_DELAY)
      {
        if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U))
        {
          ...
          //prnt("HAL_ETH_Transmit: timeout", 25);
          return HAL_ERROR;
        }
      }
    }
