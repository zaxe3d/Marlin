...
int print_stat=0;
long int ws_filesize;
long int ws_sdpos;
extern char ws_filename[];
...
void CardReader::openFileRead(const char * const path, const uint8_t subcall_type/*=0*/) {
  ...
  //if (file.open(diveDir, fname, O_READ)) {
    ...
    filesize = f_size(&MyFile);  //Elsan
    ws_filesize=filesize;        //Elsan
    strcpy(ws_filename, fname2); //Elsan
