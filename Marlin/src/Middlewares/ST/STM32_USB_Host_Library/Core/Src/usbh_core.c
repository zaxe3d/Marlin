...
{
  //prnt_els("USBH_Init enter");
  /* Check whether the USB Host handle is valid */
  if (phost == NULL)
  {
    //USBH_ErrLog("Invalid Host handle");
    //SERIAL_ECHOPGM("USBH_Init->Invalid Host handle");
...
USBH_StatusTypeDef USBH_RegisterClass(USBH_HandleTypeDef *phost, USBH_ClassTypeDef *pclass)
{
  ...
  //prnt_els("USBH_RegisterClass enter");
  ...
  //prnt_els("USBH_RegisterClass finish");
  return status;
}
...
{
  //prnt_els("USBH_Start enter");
  /* Start the low level driver  */
  USBH_LL_Start(phost);
  //SERIAL_ECHOPGM("USBH_LL_Start passed");
  //prnt_els("USBH_LL_Start passed");

  /* Activate VBUS on the port */
  USBH_LL_DriverVBUS(phost, TRUE);

  //prnt_els("USBH_Start finish");
  return USBH_OK;
}
  ...
  switch (phost->gState)
  {
    ...
    case HOST_DEV_WAIT_FOR_ATTACHMENT: /* Wait for Port Enabled */
      ...
      if (phost->device.PortEnabled == 1U)
      {
        ...
        //prnt_els("USB Device Reset Completed");
        phost->device.RstCnt = 0U;
