...
namespace ExtUI {
  ...
  /*
   * This works as G92 E(position)
   * So it doens't move back to where it was on file after extruding.
  */
  ...
  void setMaterialCustomTemp(const float ext_temp, const float bed_temp) {
    if (ext_temp > -1) // for to set individually
      material.custom_temp.ext = ext_temp;
    if (bed_temp > -1) // for to set individually
      material.custom_temp.bed = bed_temp;
  }
