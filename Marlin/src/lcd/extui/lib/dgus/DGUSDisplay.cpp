...
// Send the current print progress to the display.
void DGUSScreenVariableHandler::DGUSLCD_SendPrintProgressBarToDisplay(DGUS_VP_Variable &var) {
...
void DGUSScreenVariableHandler::DGUSLCD_SendHeaterStatusToDisplay(DGUS_VP_Variable &var) {
  ...
  void DGUSScreenVariableHandler::DGUSLCD_SD_RefreshFilelist(DGUS_VP_Variable& var, void *val_ptr) {
     filelist.reload();
  }
  ...
  void DGUSScreenVariableHandler::DGUSLCD_SD_FileSelected(DGUS_VP_Variable &var, void *val_ptr) {
    ...
    HandleUserConfirmationPopUp(VP_SD_FileSelectConfirm, nullptr, filelist.filename(), GET_TEXT(MSG_START_PRINTING), nullptr, true, true, false, true);
  }
  ...
  void DGUSScreenVariableHandler::DGUSLCD_SD_ResumePauseAbort(DGUS_VP_Variable &var, void *val_ptr) {
    ...
    switch (swap16(*(uint16_t*)val_ptr)) {
      ...
      case 1:  // Pause
        ...
        break;
      case 2:  // Abort
        ScreenHandler.HandleUserConfirmationPopUp(VP_SD_AbortPrintConfirmed, nullptr, filelist.filename(), GET_TEXT(MSG_ABORT_PRINTING), nullptr, true, true, false, true);
        break;
  ...
  void DGUSScreenVariableHandler::DGUSLCD_SD_PrintTune(DGUS_VP_Variable &var, void *val_ptr) {
    if (!ExtUI::isPrintingFromMedia()) return; // avoid race condition when user stays in this menu and printer finishes.
    GotoScreen(DGUSLCD_SCREEN_SDPRINTTUNE_ZOFFSET);
  }
...
void DGUSScreenVariableHandler::HandleCustomExtTemperatureChanged(DGUS_VP_Variable &var, void *val_ptr) {
  float custom_temp = swap16(*(uint16_t*)val_ptr);
  if (custom_temp < 180)
    custom_temp = 180; // Handle lower limit
  ExtUI::setMaterialCustomTemp(custom_temp, -1);
  (void)settings.save();
}

void DGUSScreenVariableHandler::HandleCustomBedTemperatureChanged(DGUS_VP_Variable &var, void *val_ptr) {
  float custom_temp = swap16(*(uint16_t*)val_ptr);
  if (custom_temp < 50)
    custom_temp = 50; // Handle lower limit
  ExtUI::setMaterialCustomTemp(-1, custom_temp);
  (void)settings.save();
}
...
void DGUSScreenVariableHandler::HandleEepromSaveRestoreSettings(DGUS_VP_Variable &var, void *val_ptr) {
...
void DGUSScreenVariableHandler::HandleEndPreheat(DGUS_VP_Variable &var, void *val_ptr) {
...
void DGUSScreenVariableHandler::HandleGoToCalibrationPoints(DGUS_VP_Variable &var, void *val_ptr) {
  DEBUG_ECHOLNPGM("HandleGoToCalibrationPoints");
  queue.inject_P(PSTR("G28\nG29"));
}

void DGUSScreenVariableHandler::HandleFanOnOff(DGUS_VP_Variable &var, void *val_ptr) {
...
void DGUSScreenVariableHandler::HandleGoToMaximumPoints(DGUS_VP_Variable &var, void *val_ptr) {
...
#if ENABLED(SKEW_CORRECTION)
  void DGUSScreenVariableHandler::HandleSkewVarsChanged(DGUS_VP_Variable &var, void *val_ptr) {
...
void DGUSScreenVariableHandler::HandleHeaterControl(DGUS_VP_Variable &var, void *val_ptr) {
  ...
  switch (var.VP) {
      ...
      case VP_E2_CONTROL:
    #endif
      preheat_temp = PREHEAT_PLA_HOTEND;
      break;

    case VP_BED_CONTROL:
      preheat_temp = PREHEAT_PLA_BED;
      break;
  }
  ...
  *(int16_t*)var.memadr = *(int16_t*)var.memadr > 0 ? 0 : preheat_temp;
  ...
  void DGUSScreenVariableHandler::HandlePreheat(DGUS_VP_Variable &var, void *val_ptr) {
    ...
    switch (preheat_option) {
      ...
      case 7: // Custom preheat
        ...
        bed_temp = PREHEAT_PLA_BED; // FIXME get custom bed temp here as well
        break;
      case 9: // Cool down
        e_temp = 0;
        bed_temp = 0;
  ...
  void DGUSScreenVariableHandler::HandleFirstLayerCalibrationOption(DGUS_VP_Variable &var, void *val_ptr) {
  ...
  void DGUSScreenVariableHandler::HandleFirstLayerCalibration(DGUS_VP_Variable &var) {
...
#if ENABLED(DGUS_FILAMENT_LOADUNLOAD)
  void DGUSScreenVariableHandler::HandleFilamentConfirmation(DGUS_VP_Variable &var, void *val_ptr) {
    UNUSED(var); UNUSED(val_ptr);
    filament_data.waiting_confirmation = false;
  }

  void DGUSScreenVariableHandler::HandleFilamentInit(DGUS_VP_Variable &var, void *val_ptr) {
    UNUSED(var); UNUSED(val_ptr);
    ...
    if (thermalManager.temp_hotend[filament_data.extruder].celsius >= filament_data.target_temp ||
      ...
      if (!filament_data.heated) {
        ...
        if (filament_data.action == 1) { // we are saving type if loading...
          ...
          (void)settings.save(); // save filament type to eeprom
        }
      }
  ...
  void DGUSScreenVariableHandler::HandleSDFilamentOption(DGUS_VP_Variable &var, void *val_ptr) {
  ...
  void DGUSScreenVariableHandler::HandleSDFilamentLoadUnload(DGUS_VP_Variable &var) {
