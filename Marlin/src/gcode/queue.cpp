...
void GCodeQueue::get_serial_commands() {
      ...
      if (ISEOL(serial_char)) {
        ...
        if (npos) {
          ...
          //else if (card.flag.saving && !is_M29(command)) { //Elsan dis
            //gcode_line_error(F(STR_ERR_NO_CHECKSUM), p);   //Elsan dis
            //break;                                         //Elsan dis
          //}                                                //Elsan dis
        #endif
  ...
  inline void GCodeQueue::get_sdcard_commands() {
    ...
    int sd_count = 0;
    //bool card_eof = card.eof(); //Elsan test worked.
    while (!ring_buffer.full() && !card.eof()) {
    //while (!ring_buffer.full() && !card_eof) { //Elsan test worked.
      const int16_t n = card.get();
      const bool card_eof = card.eof();
      //card_eof = card.eof(); //Elsan test worked.
      ...
      if (is_eol || card_eof) {
        ...
        //if (card_eof) card.fileHasFinished(); //Elsan test worked.
      }
      else
