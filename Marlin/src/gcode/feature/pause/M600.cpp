...
void GcodeSuite::M600() {
  ...
  if (IS_SD_PRINTING()) card.pauseSDPrint(); //Elsan test.
  print_stat=2;
  ...
  if (pause_print(retract, park_point, true/*show_lcd*/, /*unload_length DXC_PASS*/0.00)) { //Elsan unload_length from M603 U.
  //if (pause_print(retract, park_point, 0, show_lcd)) { //worked. show_lcd is in different position.
    //if (standardM600) { //Elsan dis.
      wait_for_confirmation(true, beep_count DXC_PASS); //Elsan dis for test.
      //wait_for_confirmation(false, 0); //Elsan test
      /*resume_print(
        FILAMENT_CHANGE_SLOW_LOAD_LENGTH,
        0.00, //ABS(parser.axisunitsval('L', E_AXIS, fc_settings[active_extruder].load_length)),
      ...
      //resume_print(0, 0, -retract, 0); //Elsan test.
